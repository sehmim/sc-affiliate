...
export async function fetchHostPermissions(apiUrl) {
  try {
    ...
    return campaigns.map((campaign) => (campaign.advertiserURL))
...
export async function createManifest(apiUrl) {
  ...
  fs.writeFileSync(path.resolve(__dirname, 'src', 'manifest.json'), JSON.stringify(manifest, null, 2), 'utf-8');
