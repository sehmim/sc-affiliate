...
const EmailVerificationComponent = () => {
  ...
  return(
            ...
            {
              !isVerificationCodeRequested && (
              <button disabled={loading} onClick={() => sendEmailVerificationCode()} type="button" className="btn btn-dark btn-lg fw-bold pt-2" style={{ width: '320px' }}>
                Send Code
              </button>
              )
            }
...

            {
              isVerificationCodeRequested && (
                <div>
                  <button disabled={loading} onClick={() => loginUser()} type="button" className="btn btn-dark btn-lg fw-bold" style={{ width: '320px' }}>
                    Login
                  </button>
                  <div>Did not get code? <a onClick={() => {}} href='#'>Resend Code</a></div>
                </div>
              )
            }
            </div>
